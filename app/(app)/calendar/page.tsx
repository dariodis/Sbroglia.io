...
export default function CalendarPage() {
  const { items, isLoading, error, toggleTask, deleteTask } = useTasks()

  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        <Spinner size="lg" />
        <p className="text-sm text-[var(--text-tertiary)] animate-pulse">Caricando il calendario...</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-[var(--text-tertiary)]">
          Tutte le tue attività organizzate per giorno
        </p>
      </div>
      <CalendarGrid
        items={items}
